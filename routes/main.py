...
def update_attendance(meeting_date):
    ...
    try:
        # INPUT VALIDATION
        member_id = sanitize_input(request.form.get('member_id'), max_length=100)
        status = validate_status(request.form.get('status'), ['present', 'absent', 'clear'])
...
def add_member():
    ...
    form_errors = {}
    name = validate_name(request.form.get('name', '').strip())
    age_str = request.form.get('age', '').strip()
    age = validate_age(age_str) if age_str else None
    phone_str = request.form.get('phone_number', '').strip()
    phone_number = validate_phone_number(phone_str) if phone_str else None
    
    # Validate required fields
    if not name:
        form_errors['name'] = 'Full name is required (2-100 characters, letters and spaces only)'
    
    # Validate age if provided
    if age_str and not age:
        form_errors['age'] = 'Age must be between 1 and 120'
    
    # Validate phone number if provided
    if phone_str and not phone_number:
...
def update_member(member_id):
    try:
        ...
        member_check = validate_ownership(supabase, 'cell_members', member_id, 'leader_id')
        if not member_check:
            logger.warning(f"Unauthorized update attempt: user={leader_id}, member={member_id}")
            flash('You do not have permission to update this member', 'error')
            abort(403)
        
        # INPUT VALIDATION
        name = validate_name(request.form.get('name'))
        if not name:
            flash('Invalid name. Must be 2-100 characters, letters and spaces only', 'error')
...
<<<<<<< HEAD
        ...
        flash('An error occurred while updating the member', 'error')
        return redirect(url_for('main.member_details', member_id=member_id))
...
def delete_member(member_id):
    try:
        ...
        # AUTHORIZATION CHECK: Verify member belongs to this leader
        member_record = validate_ownership(supabase, 'cell_members', member_id, 'leader_id')
        if not member_record:
            logger.warning(f"Unauthorized delete attempt: user={leader_id}, member={member_id}")
            flash('You do not have permission to delete this member', 'error')
            abort(403)
        
        member_name = member_record.get('name', 'Unknown')
